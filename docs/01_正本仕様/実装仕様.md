...
- Viewer の通常チャットでは、会話相手として `Mio` / `ChatWorker` / `Heavy` / `Wild` を選択可能にする。
- LINE 入口の `CHAT(Mio)` 固定は維持する。会話相手拡張は Viewer の明示操作に限定する。
- `ChatWorker` は `Worker` endpoint 上の短文会話 alias として扱い、`OPS` 実行とは分離する。
- `Heavy` / `Wild` は既存 route (`ANALYZE` / `WILD`) を使うが、ユーザー選択時は `agent.response from=<role> to=user` として Timeline に表示する。
...
#### 1.1.2 Target と route

| UI Target | 内部 route | provider / alias | 応答イベント | 主用途 |
|---|---|---|---|---|
| `Mio` | `CHAT` | `Chat` | `agent.response mio -> user` | 通常会話、軽い相談、最終応答 |
| `ChatWorker` | `WORKER_CHAT` | `ChatWorker` | `agent.response chatworker -> user` | 作業相談、手順整理、実行前の見積もり |
| `Heavy` | `ANALYZE` | `Heavy` | `agent.response heavy -> user` | 深い分析、設計検討、原因調査 |
| `Wild` | `WILD` | `Wild` | `agent.response wild -> user` | 創作、画像プロンプト、ComfyUI 関連 |

`WORKER_CHAT` は新規 route として扱う。既存の `OPS` は Shiro / Worker の実行系 route であり、`WORKER_CHAT` と混同しない。
...
2. 本文に明示 route command がない場合、UI Target を route に変換する。
3. UI Target が未指定または不明な場合、`Mio` / `CHAT` として扱う。

明示 route command は少なくとも以下を含む。

`/chat`, `/ops`, `/analyze`, `/heavy`, `/wild`, `/code`, `/code1`, `/code2`, `/code3`, `/code4`, `/plan`, `/research`

#### 1.1.4 ChatWorker の制約

`ChatWorker` は会話用 alias であり、以下を禁止する。
...
`ChatWorker` が実行の必要性を判断した場合は、実行せず、次に使うべき route (`OPS`, `CODE*` など) と理由を短く返す。

#### 1.1.5 Heavy / Wild の制約

`Heavy` は分析・診断・設計検討に限定し、直接実行しない。

`Wild` は創作・画像関連を担当する。ComfyUI などの画像生成は、ユーザーが明示的に生成を求めた場合のみ実行する。
...
- `from`: `mio` / `chatworker` / `heavy` / `wild`
- `to`: `user`
- `route`: `CHAT` / `WORKER_CHAT` / `ANALYZE` / `WILD`
- `job_id`: 必須
...
Viewer は Target が `ChatWorker` / `Heavy` / `Wild` の場合、送信前に LLM Ops health / status を確認し、対象 runtime が停止していれば既存の start API で起動を試みる。

起動失敗、health 不良、queue timeout、generation timeout の場合は、Mio へ暗黙フォールバックしない。エラーは対象 Target 名を含めてユーザーに表示する。

#### 1.1.8 実装対象

- Viewer UI: 通常チャット入力に Target 選択を追加または既存 Target 選択を通常チャットへ統合する。
- Viewer send payload: `target` または `model_alias` / `route_prefix` を使い、Target をサーバへ渡す。
- Viewer Timeline: `agent.response` の `from` が `chatworker` / `heavy` / `wild` で `to=user` の場合も通常会話として描画する。
- Orchestrator: `WORKER_CHAT` route を追加し、`ChatWorker` provider へ接続する。
- Agent runtime: `ChatWorker` 用の会話 agent または Shiro の会話専用 adapter を追加する。
- Tests: send payload 解決、明示 route 優先、Timeline 表示、`WORKER_CHAT` dispatch、runtime unavailable 表示を検証する。
