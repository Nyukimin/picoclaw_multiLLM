...
- **Worker 即時実行**: Coder は「提案と差分」まで、Worker が即座に実行（完全自動化、6章）。
- **セーフガード**: Git auto-commit、保護ファイルパターン、実行前サマリで安全性確保（6.3節）。
...
- **Worker の役割**: 実際の適用実行（ファイル書込み、コマンド実行）を即座に実行
- **Chat の役割**: 意思決定、ルーティング、結果返却

**重要**: Coder は原則として破壊的操作を直接実行せず、`plan` と `patch` を生成する。
Worker が生成された patch を即座に実行する（完全自動化）。
...
## 6. Worker 即時実行仕様（完全自動化）

### 6.1 基本原則
- Coder（Coder1/Coder2/Coder3）は「提案（plan）」と「差分（patch）」を生成
- **Worker が生成された patch を即座に実行**（承認フローなし）
- セーフガードで安全性を確保
- **各 Task に JobID を付与**して実行を追跡

### 6.2 JobID（Task 識別子）
- **形式**: `job_{timestamp}_{random8chars}`
- **例**: `job_1709123456_abc12345`
- **用途**: Worker 実行ジョブ（Task）の一意識別子
- **生成**: Task 作成時に自動生成
- **追跡**: ログ、実行結果、エラー報告で使用

**session_id との違い**:
- `session_id`: セッション単位（会話全体）の識別子
- `JobID`: Task 単位（個別実行）の識別子

### 6.3 標準フロー
1. Task 作成時に JobID を生成
2. Coder が提案・差分を生成
3. Worker が patch を即座に実行（JobID で追跡）
4. Git auto-commit で変更を記録
5. 結果を通知、ログ保存（JobID を含む）

### 6.4 セーフガード（安全性確保）
- **Git auto-commit**（必須）: 実行前後で自動コミット、失敗時はロールバック
- **保護ファイルパターン**: `.env*`, `*credentials*`, `*.key`, `*.pem` は変更禁止
- **実行前サマリ表示**: 実行内容を事前に確認可能
- **ワークスペース制限**: 指定ディレクトリ外への書き込み禁止
- **詳細ログ記録**: すべての実行を追跡可能

詳細: `docs/06_実装ガイド進行管理/20260228_Worker即時実行ロジック設計.md`
...
### 10.2 Worker 実行設定
- `worker.auto_commit = true`: Git auto-commit 有効化（推奨）
- `worker.commit_message_prefix = "[Worker Auto-Commit]"`: コミットメッセージのプレフィックス
- `worker.command_timeout_sec = 300`: コマンド実行タイムアウト（秒）
- `worker.git_timeout_sec = 30`: Git 操作タイムアウト（秒）
- `worker.stop_on_error = false`: エラー時の動作（false: 継続、true: 中断）
- `worker.protected_patterns = [".env*", "*credentials*", "*.key", "*.pem"]`: 保護ファイルパターン
