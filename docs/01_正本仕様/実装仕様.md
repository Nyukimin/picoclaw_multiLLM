...
- `LINE -> CHAT(Kuro)` の入口固定を維持。
- `Kuro` が会話中に `Worker/Coder` 委譲を判断するオーケストレーションを採用。
...
- 愛称（例: Kuro / Shiro / Aka）と実体LLMは可変
- 差し替えは設定ファイルで行う（`routing.llm.*`）
  - `chat_alias/chat_provider/chat_model`
  - `worker_alias/worker_provider/worker_model`
  - `coder_alias/coder_provider/coder_model`
...
- 委譲は `Kuro` の応答プロトコル（`DELEGATE` + `TASK`）でのみ実行する。
...
- Ollama呼び出し時 `options.num_ctx = 8192` をリクエスト明示
