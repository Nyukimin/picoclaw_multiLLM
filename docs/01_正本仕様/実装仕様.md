...
- Chat/Worker の Ollama モデル: `chat-v1:latest`, `worker-v1:latest`。
- Ollama リクエストに `keep_alive: -1` を付与（Chat/Worker 永続化）。
- LINE指示ごと: LLM呼び出し → ヘルスチェック（常時）→ Ollama 落ちてたら再起動 → 失敗時のみリトライ（`pkg/agent/loop.go`）。
- Coder1/Coder2 二重ルーティング（`CODE1`/`CODE2`、`coder2_alias/provider/model`）。
...
- 愛称（例: Mio / Shiro / Aka / Ao）と実体LLMは可変
...
- 委譲は `Mio` の応答プロトコル（`DELEGATE: PLAN|ANALYZE|OPS|RESEARCH|CODE|CODE1|CODE2` + `TASK`）でのみ実行する。
...
### 3.4 会話LLM提案IF
```json
{"propose_next_loop":false,"route":"CHAT|PLAN|ANALYZE|OPS|RESEARCH|CODE|CODE1|CODE2","reason":"short text","confidence":0.0}
```
