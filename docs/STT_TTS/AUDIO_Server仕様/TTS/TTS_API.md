...
- 検証コマンド例:
  - `curl -sS http://127.0.0.1:8788/health/live`
  - `curl -sS http://127.0.0.1:8788/health/ready | jq`
- 証跡:
  - [ ] 両APIレスポンスをPRに添付
  - [ ] `capabilities` 値を記録
...
- 検証コマンド例:
  - `curl -sS -X POST http://127.0.0.1:8788/synthesis -H 'Content-Type: application/json' -d '{"text":"test","voice_id":"voice_a","track":"main"}'`
  - `curl -sS -X POST http://127.0.0.1:8788/synthesize -H 'Content-Type: application/json' -d '{"text":"test","voice_id":"voice_a","session_id":"sess-1"}'`
- 証跡:
  - [ ] 両経路の成功レスポンスを添付
...
- 検証コマンド例:
  - `wscat -c ws://127.0.0.1:8788/sessions`
