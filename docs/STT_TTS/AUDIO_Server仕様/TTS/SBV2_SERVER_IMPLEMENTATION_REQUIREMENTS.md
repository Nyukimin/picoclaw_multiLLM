...
- **宅内**: ブラウザ以外のサーバ群は **同一宅内ルータ配下 LAN** に置く想定（相互にプライベート IP で届きうる）。
- **ユーザーエンド**: ブラウザ利用端末や PicoClaw 実行端末など、宅内サーバ群の外側からアクセスするクライアントを指す。
- **通常**: 構築・クライアントからの到達は **基本的に Tailscale**（`*.ts.net` 等）。**HTTP(S) / WebSocket がクライアントから到達できる**こと（TLS、CORS、Mixed Content 回避）を満たす。
- **Tailscale 不調時（最終手段）**: やむを得ない場合のみ、**ユーザーエンド以外**は **宅内 IP** での到達に切り替え可能にしておく（サーバ間・内部向け設定）。復旧後は Tailscale 前提に戻すことを推奨。
- リモートブラウザからの再生では、`audio_path` だけでは届かないことがあるため、**`audio_url` を返せる**実装・配信経路を用意することが望ましい。
...
例: `https://<tailnet-host>.ts.net:5000/synthesis`

- 上記は **通常運用の概念例**。Tailscale 不調時の緊急切替では、内部向け設定に限り宅内 IP を使用してよい。
...
`{tts.ws_url}`  
例: `wss://<tailnet-host>.ts.net:8765/sessions`
...
```json
{
  ...
  "audio_url": "https://<tailnet-host>.ts.net:8765/audio/chunk0.wav",
  "pause_after": "short"
}
```
...
```json
{
  ...
  "audio_url": "https://<tailnet-host>.ts.net:8765/audio/oneshot.wav"
}
```
