...
## 2026-06-19 追加結合ポイント（run_20260619_000000）

### 新アーキテクチャ モジュール結合

| 結合点 | 提供側 | 利用側 | 内容 |
|-------|-------|-------|------|
| `Module.Initialize(ctx, *AgentCore)` | modules/types.go | 全モジュール | Module インターフェース共通 |
| `AgentCore.Provider` | factory.go | ExecutionModule, ProposalGenerationModule, FinalDecisionModule | LLM プロバイダー [!] nil |
| `AgentCore.Bus` | factory.go | 全モジュール | MessageBus への参照 |
| `RoutingModule.RouteTask()` | worker/routing.go | loop_new.go | ルーティング決定 |
| `ExecutionModule.ExecuteTask()` | worker/execution.go | loop_new.go | タスク実行 |
| `ProposalGenerationModule.GenerateProposal()` | order/proposal.go | loop_new.go(delegateToOrderNewArch) | 提案生成 |
| `ApprovalFlowModule.ApprovePendingProposal()` | order/approval.go | — | 承認後実行なし（TODO） |
| `HeartbeatCollectorModule.ReportHeartbeat()` | worker/heartbeat.go | — | 呼び出し元なし |
| `AggregationModule.AggregateResults()` | worker/aggregation.go | loop_new.go | 結果集約 |
| `FinalDecisionModule.MakeFinalDecision()` | chat/decision.go | loop_new.go | 最終応答生成 |

### MessageBus 結合

| 結合点 | 提供側 | 利用側 | リスク |
|-------|-------|-------|-------|
| `PublishInbound()` | bus/bus.go | 全チャネルハンドラー | RLock 保持中にチャネル満杯でブロック |
| `ConsumeInbound()` | bus/bus.go | agent/loop.go | select ループで消費 |
| `PublishOutbound()` | bus/bus.go | agent/loop.go | — |
| `SubscribeOutbound()` | bus/bus.go | 全チャネルハンドラー | — |

### main.go / internal/* 結合（未実装）

| import パス | 状態 | 代替 |
|-----------|------|------|
| `internal/adapter/config` | 未実装 | `pkg/config` |
| `internal/domain/agent` | 未実装 | `pkg/agent` |
| `internal/infrastructure/llm/claude` | 未実装 | `pkg/providers/claude_provider.go` |
| `internal/application/orchestrator` | 未実装 | `pkg/agent/loop.go` |
| その他 ~35 パッケージ | 未実装 | — |
