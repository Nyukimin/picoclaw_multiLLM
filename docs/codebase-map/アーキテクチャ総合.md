...
## 2026-06-19 再解析アップデート（run_20260619_000000）

### 新規発見パッケージ（プロファイル未収録）

| パッケージ | 場所 | 役割 |
|-----------|------|------|
| `pkg/modules/chat` | chat/ | LightweightReceptionModule, FinalDecisionModule |
| `pkg/modules/worker` | worker/ | RoutingModule, ExecutionModule, AggregationModule, HeartbeatCollectorModule |
| `pkg/modules/order` | order/ | ProposalGenerationModule, CodeAnalysisModule, ApprovalFlowModule |
| `pkg/modules/types.go` | modules/ | AgentCore 共通コア・Module インターフェース |
| `pkg/heartbeat` | heartbeat/ | HeartbeatService（HEARTBEAT.md 駆動） |
| `pkg/jobid` | jobid/ | JobID Generator (job_YYYYMMDD_NNN) |
| `pkg/bus` | bus/ | MessageBus (Inbound/Outbound チャネル) |
| `pkg/state` | state/ | 最終チャネル記録（原子的永続化） |
| `pkg/health` | health/ | Ollama ヘルスチェック |
| `pkg/auth` | auth/ | OAuth 2.0 / PKCE / トークンストア |
| `pkg/tools` | tools/ | ToolRegistry, Shell/FS/Web/Edit/Subagent ツール |
| `pkg/skills` | skills/ | Skills ローダー・インストーラー |
| `pkg/cron` | cron/ | Cron スケジューラ |
| `pkg/devices` | devices/ | USB デバイスイベント（Linux） |
| `pkg/voice` | voice/ | 音声文字起こし（Transcriber） |
| `pkg/migrate` | migrate/ | 設定マイグレーション |

### 最重要 Critical 発見

**main.go と internal/ の乖離（ビルド不能状態）**

`cmd/picoclaw/main.go`（3429行）は ~40 の `internal/` パッケージを import しているが、
これらのファイルは **一切存在しない**（唯一の実在は `internal/adapter/viewer/viewer.html` のみ）。

import されている未実装パッケージ（抜粋）:
- `internal/adapter/channels`, `internal/adapter/chrome`, `internal/adapter/config`
- `internal/application/health`, `internal/application/heartbeat`, `internal/application/orchestrator`
- `internal/domain/agent`, `internal/domain/conversation`, `internal/domain/execution`
- `internal/infrastructure/llm/claude`, `internal/infrastructure/persistence/session`
- `internal/glossary` など計40+ パッケージ

**現在の状態**: コードベースはコンパイル不能。`pkg/` 層の実装（旧アーキテクチャ）は機能しているが、
`main.go` が新しい Clean Architecture (DDD) 層を参照しており、それが未実装のため結合できていない。

### 新アーキテクチャ（pkg/modules/）の実装状況

| コンポーネント | 実装 | 問題 |
|-------------|------|------|
| `AgentCore` (modules/types.go) | 完成 | — |
| `LightweightReceptionModule` | 完成 | — |
| `FinalDecisionModule` | 完成 | — |
| `RoutingModule` | 完成 | Router.Decide インターフェース互換 |
| `ExecutionModule` | 完成 | Provider が nil 時に panic (※後述) |
| `AggregationModule` | 完成 | — |
| `HeartbeatCollectorModule` | 完成 | — |
| `ProposalGenerationModule` | 完成 | — |
| `CodeAnalysisModule` | 完成 | — |
| `ApprovalFlowModule` | 部分完成 | ApprovePendingProposal が何も実行しない（TODO） |
| `factory.go` (resolveProvider) | 常に nil を返す | 新アーキテクチャ有効時にOrder実行で **nil panic** |

**factory.go L134 (resolveProvider)**:
```go
...
**order/approval.go L69-71 (ApprovePendingProposal)**:
```go
...
### アーキテクチャ並存状況（2026-06-19 時点）

```mermaid
graph TD
    main["cmd/picoclaw/main.go<br/>[!] 未実装internal/*参照"] -->|"コンパイル不能"| internal_missing["internal/* (全未実装)"]
    
    old_arch["旧アーキテクチャ<br/>pkg/agent/loop.go<br/>processMessageLegacy()"] -->|"UseNewArchitecture=false<br/>（デフォルト）"| working["実動状態"]
    
    new_arch["新アーキテクチャ<br/>pkg/agent/loop_new.go<br/>processMessageNewArch()"] -->|"UseNewArchitecture=true"| partial["部分実装<br/>provider=nil で panic"]
    
    working --> pkg_providers["pkg/providers/*"]
    working --> pkg_session["pkg/session/*"]
    working --> pkg_bus["pkg/bus/*"]
```
