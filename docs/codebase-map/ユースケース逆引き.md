...
## 2026-06-19 追加ユースケース（run_20260619_000000）

### UC6（新規）: 新アーキテクチャ（UseNewArchitecture=true）でのメッセージ処理

**前提**: `Architecture.UseNewArchitecture = true`（デフォルトは false）
...
LightweightReceptionModule.ReceiveTask()
    │  TaskPackage{JobID, Content}
    ▼
RoutingModule.RouteTask()      [pkg/modules/worker/routing.go]
    │  Route + JobID
    ▼
if Route in {CODE1,CODE2,CODE3}:
    delegateToOrderNewArch(route, task)
        │
        ├── factory.NewAgentWithModules(orderID)
        │       resolveProvider() → nil  ← [!] CRITICAL BUG
        ├── ProposalGenerationModule.GenerateProposal(task)
        └── 結果を返す
else:
    ExecutionModule.ExecuteTask(task, route)
        │  Usage nil チェック欠如  ← [!] BUG
        └── 応答文字列

AggregationModule.AggregateResults([result])
    │
    ▼
FinalDecisionModule.MakeFinalDecision(aggregated)
    │  resolveProvider() → nil  ← [!] CRITICAL BUG
    ▼
OutboundMessage 送信
```

**現在の制約**: `resolveProvider()` が nil を返すため、UC6 は**実行不能**（panic）。

**関与モジュール**:
- `pkg/agent/loop_new.go` — processMessageNewArch, delegateToOrderNewArch
- `pkg/modules/chat/reception.go` — LightweightReceptionModule
- `pkg/modules/chat/decision.go` — FinalDecisionModule
- `pkg/modules/worker/routing.go` — RoutingModule
- `pkg/modules/worker/execution.go` — ExecutionModule
- `pkg/modules/worker/aggregation.go` — AggregationModule
- `pkg/modules/order/proposal.go` — ProposalGenerationModule
- `pkg/agent/factory.go` — NewAgentWithModules, resolveProvider

---

### UC7（新規）: STT（音声入力）処理

**トリガー**: `cmd/picoclaw/main.go` の WebSocket `/stt` エンドポイント
...
RMS 計算 → 無音検出
    │
    ├── sttFinalTimeout 内に無音が続く → 文字起こしトリガー
    │       sttFinalTimeoutFromEnv（環境変数から設定）
    │       sttSilenceAbsThresholdFromEnv
    │
    ├── infer cooldown 中は無視
    │
    └── pkg/voice/transcriber.go で文字起こし
            │
            └── InboundMessage として Bus に publish → 通常の AgentLoop 処理へ
```

**設定環境変数**:
- `STT_FINAL_TIMEOUT`: 最終タイムアウト
- `STT_SILENCE_ABS_THRESHOLD`: 無音判定 RMS 閾値

**関与モジュール**:
- `cmd/picoclaw/main.go` — STT WebSocket ハンドラー
- `pkg/voice/transcriber.go` — 音声文字起こし
- `pkg/bus/bus.go` — InboundMessage publish
