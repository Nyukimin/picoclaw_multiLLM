...
本仕様は、PicoClaw の感情音声出力を既存実装へ段階導入するための **最小改修案** を定義する。  
対象は以下の3点である。

- 読み上げ対象を `agent.response` のみに限定する
- PicoClaw 側へ `EmotionState` を導入する
...
- PicoClaw が「何を読ませるか」を制御する
- PicoClaw が EmotionState を保持して TTS へ渡せるようにする
...
Emotion Planner は PicoClaw 側に配置する。  
...
1. PicoClaw が `agent.response` のみを TTS へ送る
2. PicoClaw が `EmotionState` を生成し、TTS へ送信できる
...
- Emotion Planner を完全に PicoClaw 側へ移し、TTS サーバから感情決定ロジックを除去する
