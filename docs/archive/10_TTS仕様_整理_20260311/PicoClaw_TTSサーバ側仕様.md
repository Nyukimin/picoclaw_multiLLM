...
````md
# PicoClaw リアルタイムTTS連携 仕様書

## 1. 目的

本仕様は、PicoClaw におけるリアルタイム音声出力機能を実現するための、以下 2 つの実装境界を定義する。
...
- PicoClaw の Chat は Gemma3 系ストリーミング出力を行う
...
## 5.1 PicoClaw 側の新規コンポーネント

PicoClaw 側には以下のコンポーネントを追加する。
...
## 5.3 PicoClaw 側の session ライフサイクル
...
## 5.5 PicoClaw 側の送信ルール

PicoClaw 側は token 単位ではなく、LLM 出力 delta 単位で送信してよい。
...
## 5.6 PicoClaw 側の再生ルール

PicoClaw 側は TTS Server から返る音声を session 単位で受け取る。
...
* PicoClaw 本体と TTS Server は同一ホスト
* 接続先は localhost
...
* voice_id 切替
* WAV 出力
* PicoClaw 側 Speech Bridge
* PicoClaw 側 Audio Sink
* health API
...
1. PicoClaw と TTS Server の接続は HTTP 系、実運用は WebSocket とする
...
6. PicoClaw 側は chunking や prosody を持たず、TTS Server に責務を寄せる
...
PicoClaw は LLM の文字列を逐次 TTS Server に流し、TTS Server はそれを発話単位に確定・感情付与・読み上げ整形した上で、常駐 Style-Bert-VITS2 により音声化し、chunk 単位で PicoClaw へ返却する。
