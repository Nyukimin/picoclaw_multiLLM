# RenCrow 3役固定・Spawn禁止構成 仕様（統合版）

## 0. 目的
Chat / Worker / Coder の3役固定（Spawn禁止）で、RenCrowの標準ループ（収集→分析→サマリ→次の一手）を安定運用する。  
