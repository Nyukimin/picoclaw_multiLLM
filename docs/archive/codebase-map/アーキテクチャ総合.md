...
RenCrow は、超軽量（<10MB）AI アシスタントとして、LINE/Slack からの指示を受け、複数の LLM（Ollama/Claude/OpenAI/DeepSeek）を適切にルーティングして実行する Go 製システムです。Chat/Worker/Coder の役割分離、承認フロー、日次カットオーバーによるメモリ管理を特徴とし、階層化アーキテクチャにより保守性と拡張性を確保しています。
...
RenCrow の設計は以下の 3 つの原則に基づいています：
...
RenCrow は 5 層のアーキテクチャで構成されています：
