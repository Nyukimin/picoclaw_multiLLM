...
RenCrow は階層化アーキテクチャを採用し、基盤層（infra）、ドメイン層（session/providers/approval）、制御層（agent）、拡張層（mcp）が明確に分離されている。データフローは主に Agent Loop を中心に構成され、セッション永続化、LLM 呼び出し、承認フロー、ログ出力が各モジュール間で連携する。
