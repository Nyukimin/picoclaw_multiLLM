...
目的: 高品質コーディング/推論をClaudeで行う。ただし **OAuthトークンやブラウザセッションは使わず**、**APIキー経由**に限定する。

---

## 0. 前提（決定事項）
- OAuthトークンは使わない（決定）
- PicoClaw は完全自動化を行う（Worker 即時実行）
- セーフガード（Git auto-commit、保護ファイルパターン）で安全性を確保
- Coder3は **Claude APIキー** による **APIアクセス** を行う
- Coder3は **Chrome 操作の提案（plan）** を生成できる
- Chrome 操作の実行は Worker が即座に実行（セーフガード付き）
- 詳細ログで追跡可能
...
- Worker（実行・道具係）
  ...
  - “作業を進める”、セーフガードで安全性を確保
  - 失敗時は原因を構造化し、Chatに「次の打ち手」を返す

- Coder（高難度の設計・実装係）
  - 仕様策定の骨格、複雑な推論、難しいコード生成・レビュー
  - 出力は「提案（plan）」と「適用差分（patch）」を基本とする
  - 実際の適用（書込み/実行）はWorkerが即座に担当（セーフガード付き）

### 1-2. 不変ルール（統治）
- Chatは“決める”、Workerは“動かす”、Coderは“考えて書く”
- 実行（適用）は Worker が即座に実行、Git auto-commit で記録
- Coderは原則「案と差分」まで。直接の破壊的実行はしない

### 1-3. 典型的な処理フロー（標準）
1. ユーザー → Chat：依頼
2. Chat：意図/ゴール/制約を短く確定
3. Chat → Worker：現状収集（ファイル/ログ/環境情報）
4. Chat：難度判定し、必要なら Coder3 に投げる
5. Coder3 → Chat：`plan` + `patch` + `risk`
6. Chat → Worker：patch を即座に実行（Git auto-commit 有効）
7. Worker → Chat：実行結果（成功/失敗、Git commit hash）
...
- ブラウザ UI 操作、ファイル編集、コマンド実行などの**直接実行**
  ...
  - Worker が patch を即座に実行（セーフガード付き）
- OAuth/セッション/クッキーの取得や流用（セキュリティリスク）
- 破壊的操作（削除/上書き）は **セーフガード（Git auto-commit、保護ファイルパターン）で保護**
...
2. Worker が patch を即座に実行（ファイル書込み/コマンド実行）
3. Git auto-commit で変更を自動記録
4. 結果を通知、ログへ保存

### 6-2. セーフガード（安全性確保）
- **Git auto-commit**: 実行前後で自動コミット、失敗時はロールバック
- **保護ファイルパターン**: `.env*`, `*credentials*`, `*.key`, `*.pem` は変更禁止
- **実行前サマリ表示**: 実行内容を事前に確認可能（ログ記録）
- **ワークスペース制限**: 指定ディレクトリ外への書き込み禁止
- **詳細ログ記録**: すべての実行を追跡可能

---

## 7. 入出力インタフェース（I/F）
### 7-1. 入力（Coder3へのリクエスト）
...
### 7-2. 出力（Coder3からのレスポンス）
- `job_id`: string（Task 識別子、Worker 実行追跡用）
