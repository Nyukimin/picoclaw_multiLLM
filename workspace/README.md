# workspace/ - PicoClaw ランタイムキャラクター設定

PicoClawのキャラクター人格・行動規範・スキル定義を管理するディレクトリです。
PicoClaw本体（picoClawフレームワーク由来）のランタイム設定であり、
LLMにコンテキストとして渡されます。
...
| `workspace/` | キャラクター人格・行動規範（上位設計） | PicoClawフレームワークが自動読み込み |
