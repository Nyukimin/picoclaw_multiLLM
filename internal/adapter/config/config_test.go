...
func TestLoadConfig_VisionSettings(t *testing.T) {
	tmpDir := t.TempDir()
...
`
	...
	if !cfg.Vision.Enabled {
		t.Fatal("expected vision enabled")
	}
	if cfg.Vision.BaseURL != "http://127.0.0.1:8770" {
		t.Fatalf("vision base_url = %q", cfg.Vision.BaseURL)
	}
	if cfg.Vision.EndpointPath != "/v1/vision/analyze" || cfg.Vision.HealthPath != "/health" {
		t.Fatalf("unexpected vision paths: %+v", cfg.Vision)
	}
	if cfg.Vision.ModelAlias != "Vision" || cfg.Vision.OutputFormat != "json" || cfg.Vision.DefaultLanguage != "ja" || cfg.Vision.MaxFrames != 8 {
