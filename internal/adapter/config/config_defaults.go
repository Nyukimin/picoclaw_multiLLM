...
func (c *Config) setDefaults() {
	...
	if c.Vision.EndpointPath == "" {
		c.Vision.EndpointPath = "/v1/vision/analyze"
	}
	if c.Vision.HealthPath == "" {
		c.Vision.HealthPath = "/health"
	}
	if c.Vision.TimeoutMS <= 0 {
		c.Vision.TimeoutMS = 120000
	}
	if c.Vision.ModelAlias == "" {
		c.Vision.ModelAlias = "Vision"
	}
	if c.Vision.OutputFormat == "" {
		c.Vision.OutputFormat = "json"
	}
	if c.Vision.DefaultLanguage == "" {
		c.Vision.DefaultLanguage = "ja"
	}
