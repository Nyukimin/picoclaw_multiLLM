...
func (c *Config) Validate() error {
	...
	if c.Vision.Enabled {
		base := strings.TrimSpace(c.Vision.BaseURL)
		if base == "" {
			return fmt.Errorf("vision.base_url is required when vision.enabled=true")
		}
		if !strings.HasPrefix(base, "http://") && !strings.HasPrefix(base, "https://") {
			return fmt.Errorf("vision.base_url must start with http:// or https://")
		}
		if strings.TrimSpace(c.Vision.EndpointPath) == "" {
			return fmt.Errorf("vision.endpoint_path is required when vision.enabled=true")
		}
		if c.Vision.TimeoutMS < 1000 {
			return fmt.Errorf("vision.timeout_ms must be >= 1000")
		}
	}
