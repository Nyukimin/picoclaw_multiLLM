...
func (d *RuleDictionary) Match(t task.Task) (routing.Route, float64, bool) {
	...
	// 明示的なCODEプレフィックスをチェック
	if route, matched := checkExplicitCodePrefix(message); matched {
		return route, 0.95, true
	}
...
// checkExplicitCodePrefix はCODE:, CODE1:, CODE2: などの明示的プレフィックスを検出
func checkExplicitCodePrefix(message string) (routing.Route, bool) {
	prefixes := map[string]routing.Route{
		"code:":  routing.RouteCODE2,
	...
	for prefix, route := range prefixes {
...
func isCodeEditRequest(message string) bool {
	...
	editHints := []string{
		"変更", "修正", "追記", "編集", "更新", "実装", "追加", "削除", "書き換", "直して", "作成",
	}
