...
func TestRuleDictionary_Match_CodeKeywords(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "CODE prefix defaults to CODE2",
			message:     "CODE: /tmp/example.py を作成してください",
			expectRoute: routing.RouteCODE2,
		},
		{
			name:        "CODE1 prefix selects CODE1",
			message:     "CODE1: 仕様を整理してください",
			expectRoute: routing.RouteCODE1,
		},
		{
			name:        "CODE3 prefix selects CODE3",
			message:     "CODE3: レビューしてください",
			expectRoute: routing.RouteCODE3,
		},
	}
