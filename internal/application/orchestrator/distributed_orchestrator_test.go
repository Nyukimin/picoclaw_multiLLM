...
func TestDistributedOrchestrator_ProcessMessage_CODEPrefixBypassesExplicitDCI(t *testing.T) {
	mockMio := &distMockMioAgent{chatResponse: "chat fallback"}
	mockRepo := &distMockSessionRepo{}
	router := transport.NewMessageRouter()
	defer router.Stop()
	...
	searcher := &mockDCISearcher{
		...
		result: domaindci.SearchResult{
			Pack: domaindci.EvidencePack{
				EventID: "evt_should_not_run",
				Query:   "CODE: /tmp/example.py を作成してください",
			},
		},
	}
	...
	orch.SetDCISearcher(searcher)
	resp, err := orch.ProcessMessage(context.Background(), ProcessMessageRequest{
		SessionID:   "sess-code-prefix",
		Channel:     "line",
		ChatID:      "chat-code-prefix",
		UserMessage: "CODE: /tmp/example.py を作成してください",
	})
	if err != nil {
		t.Fatalf("ProcessMessage failed: %v", err)
	}
	if searcher.calls != 0 {
		t.Fatalf("CODE: prefix must bypass explicit DCI search, got calls=%d", searcher.calls)
	}
	if mockMio.decideCalls != 1 {
		t.Fatalf("route decision should handle CODE: prefix after DCI bypass, got %d calls", mockMio.decideCalls)
	}
	if resp.Route != routing.RouteCHAT || resp.Response != "chat fallback" {
		t.Fatalf("unexpected response after DCI bypass: %+v", resp)
	}
}
...
func TestDistributedOrchestrator_SetRecallTraceStoreIgnoresTypedNil(t *testing.T) {
	mockMio := &distMockMioAgent{chatResponse: "chat fallback"}
	mockRepo := &distMockSessionRepo{}
	router := transport.NewMessageRouter()
	defer router.Stop()
	...
	searcher := &mockDCISearcher{
		...
		result: domaindci.SearchResult{
			Pack: domaindci.EvidencePack{
				EventID: "evt_dist_dci_typed_nil",
				Query:   "DCI を探して",
				Evidence: []domaindci.Evidence{{
			...
			Trace: domaindci.SearchTrace{EventID: "evt_dist_dci_typed_nil", Status: "completed"},
		},
	}
	var typedNil *mockRecallTraceStore

	orch := NewDistributedOrchestrator(mockRepo, mockMio, router, memory, nil)
	orch.SetDCISearcher(searcher)
	orch.SetRecallTraceStore(typedNil)
	resp, err := orch.ProcessMessage(context.Background(), ProcessMessageRequest{
		SessionID:   "sess-dci-typed-nil",
		Channel:     "line",
		ChatID:      "chat-dci-typed-nil",
		UserMessage: "DCI を探して",
	})
	if err != nil {
		t.Fatalf("typed nil recall trace store should be ignored, got error: %v", err)
	}
	if resp.Route != routing.RouteRESEARCH {
		t.Fatalf("route: want RESEARCH, got %s", resp.Route)
	}
