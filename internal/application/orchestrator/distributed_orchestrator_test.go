...
func TestDistributedOrchestrator_CodeRoute_FallbackToNextCoderOnError(t *testing.T) {
	mockMio := &distMockMioAgent{routeResponse: "CODE1"}
	mockRepo := &distMockSessionRepo{}
	router := transport.NewMessageRouter()
	defer router.Stop()
	...
	mioTransport := transport.NewLocalTransport()
	shiroTransport := transport.NewLocalTransport()
	coder1Transport := transport.NewLocalTransport()
	coder2Transport := transport.NewLocalTransport()
	defer mioTransport.Close()
	defer shiroTransport.Close()
	defer coder1Transport.Close()
	defer coder2Transport.Close()

	router.RegisterAgent("mio", mioTransport)
	router.RegisterAgent("shiro", shiroTransport)
	router.RegisterAgent("coder1", coder1Transport)
	router.RegisterAgent("coder2", coder2Transport)
	...
	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		msg, err := coder1Transport.Receive(ctx)
		if err != nil {
			return
		}
		errResp := domaintransport.NewErrorMessage("coder1", msg.From, msg.SessionID, msg.JobID, "coder1 failed")
		_ = shiroTransport.PutInboundMessage(errResp)
	}()

	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		msg, err := coder2Transport.Receive(ctx)
		if err != nil {
			return
		}
		okResp := domaintransport.NewMessage("coder2", msg.From, msg.SessionID, msg.JobID, "Proposal generated by coder2")
		okResp.Type = domaintransport.MessageTypeResult
		okResp.Proposal = &domaintransport.ProposalPayload{
			Plan:     "plan",
			Patch:    "patch",
			Risk:     "risk",
			CostHint: "small",
		}
		_ = shiroTransport.PutInboundMessage(okResp)
		result := domaintransport.NewMessage("shiro", "mio", msg.SessionID, msg.JobID, "fallback success")
		result.Type = domaintransport.MessageTypeResult
		_ = mioTransport.PutInboundMessage(result)
	}()
	...
	if resp.Response != "fallback success" {
		t.Fatalf("unexpected response: %s", resp.Response)
	}
}

func TestDistributedOrchestrator_CodeRoute_FallbackToDirectShiroWhenAllCodersFail(t *testing.T) {
	mockMio := &distMockMioAgent{routeResponse: "CODE1"}
	mockRepo := &distMockSessionRepo{}
	router := transport.NewMessageRouter()
	defer router.Stop()
	...
	// 直接Shiroフォールバックの結果をmioへ返す（executeToAgent(shiro)の受信先はmio）
	go func() {
		time.Sleep(20 * time.Millisecond)
		result := domaintransport.NewMessage("shiro", "mio", "test-session", "", "direct shiro success")
		result.Type = domaintransport.MessageTypeResult
		_ = mioTransport.PutInboundMessage(result)
	}()
	...
	if resp.Response != "direct shiro success" {
		t.Fatalf("unexpected response: %s", resp.Response)
	}
}
