...
func TestVerifyByContract_CodeRoute_WorkerExecutionSummary(t *testing.T) {
	c := domaincontract.Contract{Goal: "コードを修正"}
	last := autonomousapp.AttemptResult{
		Response: "実行: 1 件, 成功: 1 件, 失敗: 0 件",
		Steps:    []string{"routing.decision", "shiro.delegate", "coder.execute", "shiro.verify", "done"},
	}
	ok, kind, _ := verifyByContract(routing.RouteCODE2, c, last)
	if !ok {
		t.Errorf("Worker実行成功サマリは通過すべき (kind=%q)", kind)
	}
}
