...
func (d *distributedRouteDispatcher) ExecuteDirect(ctx context.Context, t task.Task, route routing.Route, sessionID, ttsSessionID string) (string, error) {
	...
	if route == routing.RouteWILD {
		...
		requestMsg := domaintransport.NewMessage("mio", "wild", sessionID, jid, wildTask.UserMessage())
		requestMsg.Type = domaintransport.MessageTypeTask
		d.memory.RecordMessage(requestMsg)
		streamCtx, ttsStream := d.withStreamHooks(ctx, route, jid, sessionID, t.Channel(), t.ChatID(), ttsSessionID)
		resp, err := d.wild.Generate(streamCtx, wildTask)
		if err == nil {
			resultMsg := domaintransport.NewMessage("wild", "mio", sessionID, jid, resp)
			resultMsg.Type = domaintransport.MessageTypeResult
			d.memory.RecordMessage(resultMsg)
	...
	if route == routing.RouteANALYZE {
		...
		requestMsg := domaintransport.NewMessage("mio", "heavy", sessionID, jid, heavyTask.UserMessage())
		requestMsg.Type = domaintransport.MessageTypeTask
		d.memory.RecordMessage(requestMsg)
		streamCtx, ttsStream := d.withStreamHooks(ctx, route, jid, sessionID, t.Channel(), t.ChatID(), ttsSessionID)
		resp, err := d.heavy.Generate(streamCtx, heavyTask)
		if err == nil {
			resultMsg := domaintransport.NewMessage("heavy", "mio", sessionID, jid, resp)
			resultMsg.Type = domaintransport.MessageTypeResult
			d.memory.RecordMessage(resultMsg)
