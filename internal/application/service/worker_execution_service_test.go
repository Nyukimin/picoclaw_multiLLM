...
func TestAutoCommitChanges_UsesDefaultIdentityWhenGitUserIsUnset(t *testing.T) {
	tmpDir := t.TempDir()
	...
	testFile := filepath.Join(tmpDir, "auto_commit_without_identity.txt")
	os.WriteFile(testFile, []byte("auto-commit content"), 0644)
	...
	svc := &workerExecutionService{config: cfg}
	hash, err := svc.autoCommitChanges(context.Background(), task.NewJobID(), "test auto-commit")
	if err != nil {
		t.Fatalf("autoCommitChanges should not require git user config: %v", err)
	}
	if hash == "" || hash == "no-changes" {
		t.Fatalf("expected commit hash, got %q", hash)
	}

	authorCmd := exec.Command("git", "log", "-1", "--pretty=%an <%ae>")
	authorCmd.Dir = tmpDir
	authorOutput, err := authorCmd.Output()
	if err != nil {
		t.Fatalf("git log failed: %v", err)
	}
	if got := strings.TrimSpace(string(authorOutput)); got != "RenCrow Worker <rencrow-worker@localhost>" {
		t.Fatalf("unexpected default git author: %q", got)
	}
}
