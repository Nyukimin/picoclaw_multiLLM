...
func (w *workerExecutionService) autoCommitChanges(
	...
	commitCmd.Env = gitCommitEnv(os.Environ())
...
func gitCommitEnv(base []string) []string {
	env := append([]string{}, base...)
	env = appendIfMissingEnv(env, "GIT_AUTHOR_NAME", "RenCrow Worker")
	env = appendIfMissingEnv(env, "GIT_AUTHOR_EMAIL", "rencrow-worker@localhost")
	env = appendIfMissingEnv(env, "GIT_COMMITTER_NAME", "RenCrow Worker")
	env = appendIfMissingEnv(env, "GIT_COMMITTER_EMAIL", "rencrow-worker@localhost")
	return env
}
