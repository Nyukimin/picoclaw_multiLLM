...
**特徴**:
- ✅ **統合バイナリ**: 1つの実行ファイルでサーバー・エージェント両対応
- ✅ **SSH通信**: セキュアなJSON通信（stdin/stdout）
- ✅ **負荷分散**: LLM呼び出しを複数マシンに分散
- ✅ **透明性**: すべての通信がログに記録
- ✅ **簡単セットアップ**: `install-agent.sh` で1コマンドインストール
...
```
picoclaw/
├── cmd/picoclaw/              # 統合エントリーポイント
│   ├── main.go                # サーバーモード（デフォルト）
│   └── agent.go               # エージェントモード（分散実行用）
...
│   ├── LLM運用/               # LLM運用仕様
│   └── archive/               # アーカイブ
├── install.sh                 # メインPC用インストーラー
├── install-agent.sh           # エージェントPC用インストーラー
...
# エージェントモード（分散実行用）
./picoclaw agent worker   # Worker エージェント
./picoclaw agent coder1   # Coder1 エージェント（DeepSeek）
./picoclaw agent coder2   # Coder2 エージェント（OpenAI）
./picoclaw agent coder3   # Coder3 エージェント（Claude）
```
...
install.sh は依存パッケージの自動インストール、systemd サービス設定、API キー設定を対話的に実行します。
