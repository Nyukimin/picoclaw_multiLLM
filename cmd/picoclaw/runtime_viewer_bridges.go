...
func buildViewerBridgeHandlers(
	...
	viewerSendFromOrch := func(proc messageProcessor) http.HandlerFunc {
		...
		return viewer.HandleSendWithAttachments(func(ctx context.Context, req viewer.SendRequest) (string, error) {
			message := appendVisionAnalysisToMessage(ctx, cfg.Vision, req.Message, req.Attachments, func(eventType, content string) {
				log.Printf("[Vision] %s %s", eventType, content)
				if deps.eventRelay != nil {
					deps.eventRelay.OnEvent(orchestrator.NewEvent(
						eventType, "vision", "mio", content,
						"", "", "viewer", "viewer", "viewer-user",
					))
				}
			})
			log.Printf("[main] viewerSendFromOrch: calling ProcessMessage for viewer message: %q attachments=%d", message, len(req.Attachments))
