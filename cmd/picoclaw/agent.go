...
const agentShutdownTimeout = 30 * time.Second
...
// loadDotEnvForAgent は指定パスの.envファイルを読み込み、未設定の環境変数をセット
func loadDotEnvForAgent(path string) {
...
// cmdAgent はエージェントモードでの実行
func cmdAgent() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, "Usage: picoclaw agent <agent-type>")
		fmt.Fprintln(os.Stderr, "  agent-type: worker, coder1, coder2, coder3")
		os.Exit(1)
	}
	...
	loadDotEnvForAgent(filepath.Join(homeDir, ".picoclaw", ".env"))
	loadDotEnvForAgent(filepath.Join(filepath.Dir(configPath), ".env"))
	...
	handler, err := initAgentHandler(agentType, cfg)
	if err != nil {
		log.Fatalf("Failed to initialize handler: %v", err)
	}

	log.Printf("[picoclaw agent] Starting %s agent", agentType)
	...
	if err := runAgentMessageLoop(ctx, handler); err != nil {
...
// initAgentHandler はagentTypeに応じたハンドラを初期化
func initAgentHandler(agentType string, cfg *config.Config) (AgentHandler, error) {
	switch agentType {
	case "worker":
		return initAgentWorkerHandler(cfg)
	case "coder1":
		return initAgentCoderHandler("Coder1", cfg)
	case "coder2":
		return initAgentCoderHandler("Coder2", cfg)
	case "coder3":
		return initAgentCoderHandler("Coder3", cfg)
	default:
		return nil, fmt.Errorf("unknown agent type: %s (supported: worker, coder1, coder2, coder3)", agentType)
	}
}

// initAgentWorkerHandler はWorkerハンドラを初期化
func initAgentWorkerHandler(cfg *config.Config) (*workerHandler, error) {
...
// initAgentCoderHandler はCoderハンドラを初期化
func initAgentCoderHandler(agentName string, cfg *config.Config) (*coderHandler, error) {
...
// runAgentMessageLoop はstdin/stdout上のJSON通信ループ
func runAgentMessageLoop(ctx context.Context, handler AgentHandler) error {
	...
	for scanner.Scan() {
		...
		// タイムアウト付きでハンドラ実行
		handlerCtx, handlerCancel := context.WithTimeout(ctx, agentShutdownTimeout)
