...
func (h *coderHandler) HandleMessage(ctx context.Context, msg domaintransport.Message) (domaintransport.Message, error) {
	log.Printf("[coderHandler:%s] proposal request start: job=%s from=%s to=%s msg_len=%d",
	...
	if err != nil {
		log.Printf("[coderHandler:%s] proposal generation failed: job=%s err=%v",
	...
	if p == nil {
		log.Printf("[coderHandler:%s] proposal parse failed (nil): job=%s", h.agentName, msg.JobID)
		errResp := domaintransport.NewMessage(msg.To, msg.From, msg.SessionID, msg.JobID,
	...
	if _, parseErr := patch.ParsePatch(p.Patch()); parseErr != nil {
		log.Printf("[coderHandler:%s] proposal patch invalid: job=%s err=%v patch_preview=%q",
			h.agentName, msg.JobID, parseErr, previewForLog(strings.TrimSpace(p.Patch()), 240))
		...
		retryTask := task.NewTask(jobID, retryMsg, "standalone", "agent")
		retryProposal, retryErr := h.coderAgent.GenerateProposal(ctx, retryTask)
		if retryErr != nil {
			errResp := domaintransport.NewMessage(msg.To, msg.From, msg.SessionID, msg.JobID,
	...
	log.Printf("[coderHandler:%s] proposal generated: job=%s plan_len=%d patch_len=%d risk_len=%d cost_len=%d",
