...
## ⚠️ 0. 実装開始前の必須確認事項（CRITICAL）

### 0.1 go.mod モジュールパスの検証（最優先）

**❌ 絶対にやってはいけないこと**:
- go.mod のモジュールパスを**確認せずに**実装を開始する
- fork元のモジュールパス（`github.com/sipeed/picoclaw`）をそのまま使う
...
# 1. リポジトリURLを確認
git remote -v
# => origin  https://github.com/Nyukimin/picoclaw_multiLLM.git

# 2. go.mod のモジュールパスを確認
grep "^module " go.mod
# => module github.com/Nyukimin/picoclaw_multiLLM

# 3. 一致していることを確認
# 一致していなければ即座にgo.modを修正
```

**正しいモジュールパス**: `github.com/Nyukimin/picoclaw_multiLLM`

**なぜこれが最優先か**:
- 間違ったモジュールパスで実装すると、全ファイルのimport文が誤る
- 後から修正すると全ファイル修正が必要（重大な手戻り）
- ビルドエラー、テストエラーの原因になる
...
- [ ] `git remote -v` でリポジトリURL確認
- [ ] `go.mod` のモジュールパスが正しいことを確認
- [ ] 不一致があれば即座に修正・コミット
- [ ] `go build ./...` でビルド成功を確認

**このチェックリストを飛ばした場合のペナルティ**: 全実装のやり直し
...
- ❌ **同じアプローチで3回以上トライアンドエラーする**
  - 2回試して解決しなければ、立ち止まって原因を分析し直す
  - 同じ手法の微修正を繰り返さず、別の仮説を立てるかユーザーに相談する

- ❌ **指摘を受けた箇所だけ直して、同じ問題がある他の箇所を放置する**
  - 指摘は「原則」として吸収し、同じ原則が当てはまる全箇所に適用する

- ❌ **既存コードの間違ったパターンを踏襲して追加コードを書く**
  - 問題のあるパターンに合わせるのではなく、根本から正す提案をする
  - 一般的・標準的な手法が存在するなら、それを最初に採用する

#### 4.1.7 E2Eテスト

- ❌ **E2Eテストで本番と異なる設定経路を使う**
  - ハードコードや `os.Getenv` 直読みではなく、本番コードと同じ設定読込パス（`.env` → env vars → `config.yaml` `${ENV_VAR}` → Config struct）を通す
  - E2Eの目的は「本番環境が正しく動くことの証明」であり、テスト独自の経路を作ると意味がない
