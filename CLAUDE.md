...
**補助参照**:
- `docs/実装仕様_分散実行_v4.md` - 分散実行対応版
- `docs/実装仕様_会話LLM_v5.md` + `docs/実装仕様_会話エンジン_v5.1.md` - 会話システム
- `docs/LLM運用/` - LLM 固有の運用ルール
- `docs/移植仕様.md` - 機能拡張計画（OpenClaw 移植）

**基盤思想**:
- `TOOL_CONTRACT.md` - ツール契約（ルート直下）
- `docs/会話LLM仕様_v1.0.md` - 会話設計原則
- `docs/拡張設計仕様.md` - データ基盤拡張ルール
...
- ❌ **仕様を読まずに実装する**（必ず `docs/実装仕様_v3.md` を参照）
...
- ❌ **アーカイブファイルを直接編集する**（`docs/archive/` は読み取り専用）
...
1. **仕様確認**: `docs/仕様.md` で要件を確認
2. **実装仕様確認**: `docs/実装仕様_v3.md` を参照（必要に応じて v4/v5/v5.1 も）
3. **移植仕様確認**: `docs/移植仕様.md` で Phase と優先度を確認
4. **実装**: 仕様に従って段階的に実装
...
- **要件仕様**: `docs/仕様.md`（上位要求）
- **実装仕様 v3.0**: `docs/実装仕様_v3.md`（Clean Architecture実装の一次参照）
...
- **会話LLM仕様**: `docs/会話LLM仕様_v1.0.md`（会話システムの設計原則）
- **拡張設計仕様**: `docs/拡張設計仕様.md`（エンタメDB基盤の成長戦略）

### 6.3 運用仕様

- **移植仕様**: `docs/移植仕様.md`（OpenClaw機能→PicoClaw移植計画、6Phase）
